    ...
    switch (format.toLowerCase()) {
      ...
      case 'jpeg':
        for(let i = 0; i < req.files.length; i++) {
          ...
          convertedImageBuffer = await sharp(req.files[i].path).resize(600, 600, { fit: sharp.fit.inside, withoutEnlargement: true }).jpeg({ lossless: true, quality: 100}).toBuffer()
      ...
      case 'png':
        for(let i = 0; i < req.files.length; i++) {
          ...
          convertedImageBuffer = await sharp(req.files[i].path).resize(600, 600, { fit: sharp.fit.inside, withoutEnlargement: true }).png({ lossless: true, quality: 100}).toBuffer()
          fs.writeFileSync(`${outputPath}${small}`, convertedImageBuffer);
          image.small = `${process.env.URL}${small}`
          files.push(image)
          req.images = files;
      ...
      case 'webp':
        for(let i = 0; i < req.files.length; i++) {
          ...
          convertedImageBuffer = await sharp(req.files[i].path).resize(600, 600, { fit: sharp.fit.inside, withoutEnlargement: true }).webp({ lossless: true, quality: 100}).toBuffer()
          fs.writeFileSync(`${outputPath}${small}`, convertedImageBuffer);
          image.small = `${process.env.URL}${small}`
          files.push(image)
          req.images = files;
